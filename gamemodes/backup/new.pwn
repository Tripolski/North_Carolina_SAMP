...
{
// {"3D TEXT",}
	{"Завод\nПроизводственный цех",-1836.6434,110.4750,15.1172, 0, 1401.6876,-26.0865,3001.4951,180.000, 1, 1}, // Вход в помещения офиса завода
...
{
	new text[55];
	format(text, sizeof text, "{FFA500}Ткань\nНа складе: %d шт.", PW[factory]);
...
stock LoadObjects()
{
	...
	#include <objects/OBJ_FinalBiuld.pwn>
	...
	switch(dialogid)
	{
		...
		case dialog_login:
		{
		    ...
		    if(response)
		    {
				...
				{
					case 'А'..'Я', 'а'..'я', ' ':
					return SPD(playerid,dialog_login,DIALOG_STYLE_INPUT,"{FFA500}Авторизация","{ffffff}Введите ваш пароль","Далее","Закрыть");
				}
		        if(!strlen(inputtext)) SPD(playerid,dialog_login,DIALOG_STYLE_INPUT,"{FFA500}Авторизация","{ffffff}Введите ваш пароль","Далее","Закрыть");
  	...
  	case dialog_errorpass:
   		{
			...
			SPD(playerid,dialog_login,DIALOG_STYLE_INPUT,"{FFA500}Авторизация","{ffffff}Введите ваш пароль","Далее","Закрыть");
     	}
  	case dialog_job:
  	    {
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
	...
	if(playertextid == login_rp_PTD[playerid][1]) ShowPlayerDialog(playerid,dialog_login,DIALOG_STYLE_INPUT,"{FFA500}Авторизация","{ffffff}Введите пароль от аккаунта","Далее","Отмена");
	return 1;
}
